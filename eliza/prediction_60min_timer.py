...
def predict(timeInterval,model,column):
	...
	if column == 'close':
		close_price = preds
		close_time =  timestamp_datetime(cur_time + 3600 ) 
		print(cur_time)
	elif column == 'high':
		high_price = preds
	else:
...
def insert(time,high,low,close):
	#注意表名固定了
	insert_sql = 'insert into forecast_data_60mins (time,high,low,close) values (%s,%s,%s,%s)'
	select_sql = 'select * from forecast_data_60mins where time = %s'
	update_sql = 'update forecast_data_60mins set high = %s, low = %s, close = %s where time = %s'
	global cursor
	try:
		...
		print("insert table forecast_data_60mins failed.")
		print("Error: {}".format(err.msg))
		sys.exit()
#要将此函数设置为定时执行   
def runPredict():
	begin = time.time()
	predict('60min',model_close,'close')
	predict('60min',model_high,'high')
	predict('60min',model_low,'low')
	insert(close_time,str(high_price),str(low_price),str(close_price))
	end = time.time()
	global timer
	timer = threading.Timer(3600 - end + begin, runPredict)
	timer.start()
...
data = hb.get_kline('btcusdt', '60min', size=2000)
produce_model('60min','close')
produce_model('60min','high')
produce_model('60min','low')
