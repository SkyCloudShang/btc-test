...
def predict(timeInterval,model,column):
	...
	if column == 'close':
		close_price = preds
		close_time =  timestamp_datetime(cur_time + 900 ) 
		print(cur_time)
	elif column == 'high':
		high_price = preds
	else:
...
def runPredict():
	begin = time.time()
	predict('15min',model_close,'close')
	predict('15min',model_high,'high')
	predict('15min',model_low,'low')
	insert(close_time,str(high_price),str(low_price),str(close_price))
	end = time.time()
	global timer
	timer = threading.Timer(900 - end + begin, runPredict)
	timer.start()
...
produce_model('15min','close')
produce_model('15min','high')
produce_model('15min','low')
